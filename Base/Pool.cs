...
namespace Tools.Base
{
    ...
    public class Poolable<T> where T : class, new()
    {
        private static Pool<T> pool = new Pool<T>(10,
                   () => new T { });

        public static T RentFromPool()
        {
            // try to get from the pool; only allocate new obj if necessary
            return pool.Pop() ?? new T();
        }

        public static void ReturnToPool(T m)
        {
            pool.Push(m);
        }
